/** DateDrop Concierge email templates. Plain text and HTML, one source of truth. */
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border:1px solid ${BRAND.border};border-radius:18px;overflow:hidden;">
        <tr><td style="padding:28px 32px 8px 32px;">
          <div style="font-size:13px;letter-spacing:0.14em;text-transform:uppercase;color:${BRAND.muted};font-weight:600;">DateDrop</div>
        </td></tr>
      ...
      <div style="max-width:520px;margin:18px auto 0;font-size:12px;line-height:1.6;color:${BRAND.muted};text-align:left;">
        ${footerNote}<br>
        You can change what DateDrop emails you, or stop receiving DateDrops entirely, in Settings.
      </div>
    </td></tr>
  </table>
</body></html>`;
...
You're set up. From here, DateDrop only needs one thing from you: when you're free.
...
— DateDrop Concierge`;
  ...
  return {
    ...
    html: shell(
      ...
      "You're getting this because you created a DateDrop account.",
    ),
  };
}

export function invitationEmail(d: DropEmailData): EmailContent {
  const subject = `You've got a DateDrop — ${d.when}, ${d.area}`;
  ...
  return {
    ...
    html: shell(
      h1("You've got a DateDrop.") +
        ...
        `<div style="margin-top:4px;">${button(d.url, "Open your DateDrop")}</div>` +
      ...
      "You're getting this because you asked DateDrop to find you a date.",
    ),
  };
}
  ...
  return {
    ...
    html: shell(
      h1("You're in.") +
        ...
        `<div style="margin-top:16px;">${button(d.url, "View your DateDrop")}</div>`,
      "You're getting this because you accepted a DateDrop.",
    ),
  };
}
...
You won't need to exchange numbers. If something changes, use the DateDrop page — there are a few one-tap messages for exactly that.
  ...
  return {
    ...
    html: shell(
      h1("It's a date.") +
        ...
        `<p style="margin:18px 0 0 0;font-size:13px;line-height:1.6;color:${BRAND.muted};">You won't need to exchange numbers. If something changes, use the DateDrop page — there are a few one-tap messages for exactly that.</p>`,
      "You're getting this because your DateDrop was confirmed.",
    ),
  };
}
...
export function reminderEmail(
  ...
  const subject = `Tomorrow: your DateDrop in ${d.area}`;
  const text = `Hi ${d.firstName},

Quick reminder — your DateDrop is coming up.
...
If you can't make it, cancel from the DateDrop page so we can tell them properly.
  ...
  return {
    ...
    html: shell(
      h1("Coming up.") +
        ...
        `<p style="margin:18px 0 0 0;font-size:13px;line-height:1.6;color:${BRAND.muted};">If you can't make it, cancel from the DateDrop page so we can tell them properly.</p>`,
      "You're getting this because you have a confirmed DateDrop.",
    ),
  };
}

export function expiredEmail(d: DropEmailData): EmailContent {
  const subject = `We cancelled your ${d.when} DateDrop`;
  const text = `Hi ${d.firstName},
...
Your DateDrop on ${d.when} in ${d.area} was cancelled.
...
— DateDrop Concierge`;
  ...
  return {
    ...
    text,
    html: shell(
      h1("That DateDrop was cancelled.") +
        p(`${d.when} · ${d.area}, ${d.city}`) +
        p(d.reason) +
        p("Your availability is still open — we'll keep looking.") +
        `<div style="margin-top:16px;">${button(d.url, "Open DateDrop")}</div>`,
      "You're getting this because you were part of a cancelled DateDrop.",
    ),
  };
}

export function updatedEmail(d: DropEmailData): EmailContent {
  const subject = `Your ${d.when} DateDrop changed a little`;
  const text = `Hi ${d.firstName},

We adjusted your DateDrop so it works for both of you.
...
— DateDrop Concierge`;
  ...
  return {
    ...
    text,
    html: shell(
      h1("Small change to your DateDrop.") +
      ...
      "You're getting this because you're part of this DateDrop.",
    ),
  };
}
...
export function safetyEmail(args: {
  ...
  return {
    ...
    html: shell(
      ...
      "You're getting this because it affects your safety on DateDrop.",
    ),
  };
}
    ...
    html: shell(
      h1("We got your message.") +
        ...
        `<ul style="margin:0 0 14px 0;padding-left:18px;font-size:15px;line-height:1.7;color:${BRAND.ink};">
           ...
           <li>Pause DateDrops entirely</li>
         </ul>` +
        `<div style="margin-top:4px;">${button(args.url, "Open DateDrop")}</div>` +
      ...
      "You're getting this because you emailed DateDrop Concierge.",
    ),
  };
}
