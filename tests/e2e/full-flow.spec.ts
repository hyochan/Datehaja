...
test("account to private agent debrief and human consent", async ({ page }) => {
  ...
  const email =
    process.env.DATEHAJA_E2E_EMAIL ?? `agent-e2e-${suffix}@example.test`;
  ...
  await page
    .getByRole("button", { name: /Tell My Agent who to find/i })
    .click();
  ...
  await page
    .getByLabel("Tell Juno's Agent the version close friends know")
    .fill(
  ...
  await expect(
    page.getByRole("heading", { name: "Juno's Agent", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("group", { name: /Juno's Agent's private room/i }),
  ).toBeVisible();
  await expect(page.getByText("What Juno's Agent remembers")).toBeVisible();
  await expect(
    page.getByRole("img", { name: "Juno's Agent, your dating agent" }),
  ).toBeVisible();

  await page
    .getByLabel("Message Juno's Agent")
    .fill(
      "People often mistake my quietness for disinterest. Please remember that.",
    );
  await page.getByRole("button", { name: "Send", exact: true }).click();
  await expect(
    page.getByText(/Juno's Agent is thinking, not typing/i),
  ).toBeVisible();
  await expect(
    page
      .locator(".agent-bubble")
      .filter({ hasNotText: /I'm Juno's Agent/ })
      .last(),
  ).toBeVisible({ timeout: 60_000 });

  await page
    .getByRole("button", { name: /Send Juno's Agent scouting/i })
    .click();
  ...
  await expect(
    page.getByRole("region", { name: /Juno's Agent and .* in/i }),
  ).toBeVisible();
