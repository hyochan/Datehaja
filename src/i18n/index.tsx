...
> = {
  ...
  "Talk with {agent}": ["{agent}에게 말 걸기", "{agent}と話す", "Mit {agent} sprechen", "Parler avec {agent}", "Praat met {agent}", "Prata med {agent}"],
  "{voice} voice": ["{voice} 말투", "{voice}な話し方", "Stimme: {voice}", "Voix {voice}", "{voice} stem", "{voice} röst"],
  "{mode} mode": ["{mode} 모드", "{mode}モード", "Modus: {mode}", "Mode {mode}", "{mode}-modus", "{mode}-läge"],
  "private memory": ["나만의 비공개 기억", "非公開メモリー", "private Erinnerung", "mémoire privée", "privégeheugen", "privat minne"],
  warm: ["따뜻한", "温かな", "warm", "chaleureuse", "warm", "varm"],
  playful: ["유쾌한", "遊び心のある", "spielerisch", "joueuse", "speels", "lekfull"],
  direct: ["솔직한", "率直な", "direkt", "directe", "direct", "direkt"],
  quiet: ["차분한", "穏やかな", "ruhig", "calme", "rustig", "lugn"],
  observe: ["관찰", "観察", "beobachten", "observer", "observeren", "observera"],
  suggest: ["제안", "提案", "vorschlagen", "suggérer", "voorstellen", "föreslå"],
  advocate: ["적극 추천", "後押し", "befürworten", "défendre", "aanmoedigen", "förespråka"],
  "Live from the date world": ["데이트 월드에서 실시간으로", "デートワールドからライブ", "Live aus der Date-Welt", "En direct du monde des rendez-vous", "Live uit de datewereld", "Live från dejtvärlden"],
  "Scout Pass ready": ["Scout Pass 준비 완료", "Scout Pass 準備完了", "Scout Pass bereit", "Scout Pass prêt", "Scout Pass klaar", "Scout Pass redo"],
  "Ready to search": ["탐색 준비 완료", "探索の準備完了", "Bereit zur Suche", "Prêt à chercher", "Klaar om te zoeken", "Redo att söka"],
  "{agent} is out meeting someone.": ["{agent}, 지금 데이트 중이에요.", "{agent}は今、誰かに会っています。", "{agent} trifft gerade jemanden.", "{agent} rencontre quelqu’un en ce moment.", "{agent} ontmoet nu iemand.", "{agent} träffar någon just nu."],
  "{agent} brought something home.": ["{agent}의 데이트 리포트가 도착했어요.", "{agent}が話を持ち帰りました。", "{agent} hat etwas mitgebracht.", "{agent} a rapporté quelque chose.", "{agent} heeft iets meegebracht.", "{agent} har tagit med sig något hem."],
  "Send {agent} out to meet someone.": ["{agent}를 새로운 만남에 보내세요.", "{agent}を新しい出会いへ送り出しましょう。", "Schick {agent} los, um jemanden zu treffen.", "Envoyez {agent} rencontrer quelqu’un.", "Stuur {agent} eropuit om iemand te ontmoeten.", "Skicka ut {agent} för att träffa någon."],
  "Drop in now. The six moments are saved as they happen, then {agent} returns with a private read.": ["지금 들어가 보세요. 여섯 장면이 차례로 기록되고, 끝나면 {agent}가 비공개 소감을 들려줘요.", "今すぐのぞいてみましょう。6つの場面が記録され、最後に{agent}が非公開の感想を届けます。", "Schau jetzt vorbei. Sechs Momente werden live gespeichert, danach kehrt {agent} mit einer privaten Einschätzung zurück.", "Entrez maintenant. Les six moments sont enregistrés, puis {agent} revient avec son avis privé.", "Kijk nu mee. Zes momenten worden live bewaard, daarna komt {agent} terug met een privé-oordeel.", "Titta in nu. Sex ögonblick sparas, sedan återvänder {agent} med en privat bedömning."],
  "{agent} checks the brief, meets a compatible Agent, and returns with an honest recommendation.": ["{agent}: 내 브리프를 들고 잘 맞을 법한 에이전트를 만난 뒤, 솔직한 추천을 가지고 돌아와요.", "{agent}が希望を確認し、相性のよいエージェントと会って、率直な提案を持ち帰ります。", "{agent} prüft das Briefing, trifft einen passenden Agent und kehrt mit einer ehrlichen Empfehlung zurück.", "{agent} relit votre brief, rencontre un Agent compatible et revient avec une recommandation honnête.", "{agent} bekijkt je briefing, ontmoet een passende Agent en komt terug met een eerlijk advies.", "{agent} läser briefen, träffar en passande Agent och återvänder med ett ärligt råd."],
  "Watch the date live →": ["데이트 실시간으로 보기 →", "デートをライブで見る →", "Date live ansehen →", "Voir le rendez-vous en direct →", "Bekijk de date live →", "Se dejten live →"],
  "Read the private debrief →": ["비공개 리포트 읽기 →", "非公開レポートを読む →", "Privaten Bericht lesen →", "Lire le compte rendu privé →", "Lees het privéverslag →", "Läs den privata rapporten →"],
  "Checking Scout Pass…": ["Scout Pass 확인 중…", "Scout Passを確認中…", "Scout Pass wird geprüft…", "Vérification du Scout Pass…", "Scout Pass controleren…", "Kontrollerar Scout Pass…"],
  "Send {agent} scouting →": ["{agent} 탐색 보내기 →", "{agent}を探索へ →", "{agent} losschicken →", "Envoyer {agent} en exploration →", "Stuur {agent} op pad →", "Skicka ut {agent} →"],
  "Unlock scouting →": ["탐색 시작하기 →", "探索を解放 →", "Suche freischalten →", "Débloquer l’exploration →", "Ontgrendel zoeken →", "Lås upp sökning →"],
  "Development demo pass · no charge": ["개발 데모 패스 · 결제 없음", "開発デモパス・課金なし", "Entwicklungs-Demopass · kostenlos", "Pass démo de développement · gratuit", "Ontwikkeldemopas · gratis", "Utvecklingsdemo · kostnadsfri"],
  "The pass funds the search, not a guaranteed match. Contact stays sealed.": ["패스는 탐색 비용이며 매칭을 보장하지 않아요. 연락처는 계속 비공개예요.", "パスは探索のためのもので、マッチを保証しません。連絡先は非公開です。", "Der Pass finanziert die Suche, nicht ein garantiertes Match. Kontaktdaten bleiben versiegelt.", "Le pass finance la recherche, sans garantir un match. Les coordonnées restent scellées.", "De pas betaalt de zoektocht, niet een gegarandeerde match. Contact blijft afgeschermd.", "Passet finansierar sökningen, inte en garanterad match. Kontaktuppgifter förblir dolda."],
  "How your scout works": ["에이전트 탐색 흐름", "探索の流れ", "So funktioniert die Suche", "Comment fonctionne l’exploration", "Zo werkt de zoektocht", "Så fungerar sökningen"],
  "Briefed by you": ["내가 알려준 것", "あなたからのブリーフ", "Von dir gebrieft", "Briefé par vous", "Door jou gebrieft", "Briefad av dig"],
  "Agent, ideal person, honest profile": ["에이전트 · 이상형 · 솔직한 내 모습", "エージェント・理想の相手・正直な自分", "Agent, Wunschperson, ehrliches Profil", "Agent, personne idéale, profil sincère", "Agent, ideale persoon, eerlijk profiel", "Agent, ideal person, ärlig profil"],
  "Scouts the agent world": ["데이트 월드 탐색", "エージェント世界を探索", "Erkundet die Agent-Welt", "Explore le monde des Agents", "Verkent de agentwereld", "Utforskar agentvärlden"],
  "Watch the date unfold moment by moment": ["여섯 장면을 실시간으로 지켜봐요", "6つの場面をリアルタイムで見る", "Das Date Moment für Moment verfolgen", "Suivez les six moments en direct", "Bekijk de zes momenten live", "Följ de sex ögonblicken live"],
  "Returns with a case": ["솔직한 판단과 함께 귀환", "率直な判断とともに帰還", "Kehrt mit einer Einschätzung zurück", "Revient avec un avis", "Keert terug met een oordeel", "Återvänder med en bedömning"],
  "Private debrief, then two human yeses": ["비공개 리포트, 그리고 두 사람의 동의", "非公開レポート、そして二人の同意", "Privater Bericht, dann zwei menschliche Jas", "Compte rendu privé, puis deux accords humains", "Privéverslag, daarna twee menselijke ja’s", "Privat rapport, sedan två mänskliga ja"],
  "Private line": ["나와 에이전트만의 대화", "あなたとエージェントだけの会話", "Private Verbindung", "Ligne privée", "Privélijn", "Privat linje"],
  "only you can read this": ["나만 볼 수 있어요", "あなただけが読めます", "nur du kannst das lesen", "vous seul pouvez lire ceci", "alleen jij kunt dit lezen", "bara du kan läsa detta"],
  "{agent} is thinking, not typing…": ["{agent}, 답을 고르고 있어요…", "{agent}は答えを考えています…", "{agent} denkt nach…", "{agent} réfléchit…", "{agent} denkt na…", "{agent} tänker…"],
  "Try asking": ["이렇게 물어보세요", "こんなふうに聞いてみる", "Frag zum Beispiel", "Essayez de demander", "Probeer te vragen", "Prova att fråga"],
  "What do you understand about me so far?": ["지금까지 나를 어떻게 이해했어?", "今のところ私をどう理解してる？", "Was hast du bisher über mich verstanden?", "Qu’as-tu compris de moi jusqu’ici ?", "Wat begrijp je tot nu toe van mij?", "Vad har du förstått om mig hittills?"],
  "What will you look for on my behalf?": ["나 대신 어떤 사람을 찾을 거야?", "私のためにどんな人を探す？", "Wonach wirst du für mich suchen?", "Que vas-tu chercher pour moi ?", "Waar ga je namens mij naar zoeken?", "Vad kommer du att leta efter åt mig?"],
  "Ask me something that would change your search.": ["다음 탐색이 달라질 만한 걸 하나 물어봐.", "次の探索が変わる質問をして。", "Frag mich etwas, das deine Suche verändern würde.", "Pose-moi une question qui changerait ta recherche.", "Vraag iets dat je zoektocht zou veranderen.", "Fråga mig något som skulle ändra din sökning."],
  "Tell your agent what people usually misunderstand about you…": ["사람들이 나를 자주 오해하는 점을 말해보세요…", "人によく誤解されることを話してみて…", "Erzähl deinem Agent, was andere oft an dir missverstehen…", "Dites à votre Agent ce que les autres comprennent souvent mal…", "Vertel je Agent wat mensen vaak verkeerd begrijpen…", "Berätta vad andra ofta missförstår om dig…"],
  Send: ["보내기", "送信", "Senden", "Envoyer", "Versturen", "Skicka"],
  you: ["나", "あなた", "du", "vous", "jij", "du"],
  "next date": ["다음 데이트", "次のデート", "nächstes Date", "prochain rendez-vous", "volgende date", "nästa dejt"],
  "What {agent} remembers": ["{agent}의 기억", "{agent}が覚えていること", "Was {agent} behält", "Ce que {agent} retient", "Wat {agent} onthoudt", "Vad {agent} minns"],
  "Nothing distilled yet. Your corrections will become private memory here.": ["아직 정리된 기억이 없어요. 내가 바로잡아 준 내용이 여기에 비공개로 쌓여요.", "まだ整理された記憶はありません。あなたの訂正がここに非公開で蓄積されます。", "Noch nichts verdichtet. Deine Korrekturen werden hier zur privaten Erinnerung.", "Rien n’est encore synthétisé. Vos corrections formeront ici une mémoire privée.", "Nog niets samengevat. Je correcties worden hier privégeheugen.", "Inget har sammanfattats än. Dina rättelser blir privat minne här."],
  "Learned from Agent dates": ["에이전트 데이트에서 배운 점", "エージェントデートで学んだこと", "Aus Agent-Dates gelernt", "Appris des rendez-vous d’Agents", "Geleerd van agentdates", "Lärt från Agent-dejter"],
  "Private to you": ["나만 볼 수 있음", "あなたにだけ非公開", "Privat für dich", "Privé pour vous", "Privé voor jou", "Privat för dig"],
  "Please correct or forget this memory: ": ["이 기억을 바로잡거나 잊어줘: ", "この記憶を直すか忘れて：", "Bitte korrigiere oder vergiss diese Erinnerung: ", "Corrige ou oublie ce souvenir : ", "Corrigeer of vergeet deze herinnering: ", "Rätta eller glöm detta minne: "],
  "Correct this memory →": ["기억 바로잡기 →", "記憶を直す →", "Erinnerung korrigieren →", "Corriger ce souvenir →", "Herinnering corrigeren →", "Rätta minnet →"],
  "Field notes": ["에이전트의 기록", "エージェントの記録", "Feldnotizen", "Carnet de terrain", "Veldnotities", "Fältanteckningar"],
  "Agent dates": ["에이전트 데이트", "エージェントデート", "Agent-Dates", "Rendez-vous d’Agents", "Agentdates", "Agent-dejter"],
  "{count} total": ["총 {count}개", "全{count}件", "{count} insgesamt", "{count} au total", "{count} totaal", "{count} totalt"],
  "No stories yet.": ["아직 데이트 기록이 없어요.", "まだデートの記録はありません。", "Noch keine Geschichten.", "Pas encore d’histoire.", "Nog geen verhalen.", "Inga berättelser än."],
  "Your first agent date will appear here as a transcript and an honest private debrief.": ["첫 에이전트 데이트가 시작되면 대화 기록과 솔직한 비공개 리포트가 여기에 남아요.", "最初のエージェントデートは会話記録と率直な非公開レポートとしてここに表示されます。", "Dein erstes Agent-Date erscheint hier als Transkript und ehrlicher privater Bericht.", "Votre premier rendez-vous d’Agents apparaîtra ici avec la conversation et un compte rendu privé.", "Je eerste agentdate verschijnt hier als transcript en eerlijk privéverslag.", "Din första Agent-dejt visas här som transkript och ärlig privat rapport."],
  "Preparing the date world": ["데이트 월드 준비 중", "デートワールドを準備中", "Date-Welt wird vorbereitet", "Préparation du monde du rendez-vous", "Datewereld wordt voorbereid", "Förbereder dejtvärlden"],
  "{agent} is on a date": ["{agent}가 데이트 중", "{agent}はデート中", "{agent} ist auf einem Date", "{agent} est en rendez-vous", "{agent} is op date", "{agent} är på dejt"],
  "Private debrief ready": ["비공개 리포트 도착", "非公開レポートが完成", "Privater Bericht bereit", "Compte rendu privé prêt", "Privéverslag klaar", "Privat rapport klar"],
  "Your agent says meet": ["에이전트의 제안 · 만나보기", "エージェントの提案・会ってみる", "Dein Agent empfiehlt ein Treffen", "Votre Agent conseille de se rencontrer", "Je Agent adviseert een ontmoeting", "Din Agent föreslår ett möte"],
  "Your agent says pass": ["에이전트의 제안 · 이번엔 패스", "エージェントの提案・今回は見送る", "Dein Agent rät zum Passen", "Votre Agent conseille de passer", "Je Agent adviseert over te slaan", "Din Agent föreslår att avstå"],
  "Date interrupted": ["데이트가 중단됐어요", "デートが中断されました", "Date unterbrochen", "Rendez-vous interrompu", "Date onderbroken", "Dejten avbröts"],
  "another agent": ["다른 에이전트", "別のエージェント", "ein anderer Agent", "un autre Agent", "een andere Agent", "en annan Agent"],
  "explicitly AI · private simulation": ["AI 에이전트 · 비공개 시뮬레이션", "AIエージェント・非公開シミュレーション", "klar als KI gekennzeichnet · private Simulation", "IA clairement indiquée · simulation privée", "duidelijk AI · privésimulatie", "tydligt AI · privat simulering"],
  "World spark": ["오늘의 영감", "今日のきっかけ", "Inspiration der Welt", "Inspiration du monde", "Wereldvonk", "Världens gnista"],
  "Agent scouting journey": ["에이전트 탐색 여정", "エージェント探索の旅", "Reise des Agents", "Parcours d’exploration", "Zoektocht van de Agent", "Agentens sökresa"],
  "Left home": ["방을 나섰어요", "部屋を出た", "Von zu Hause aufgebrochen", "A quitté sa chambre", "Vertrokken", "Lämnade hemmet"],
  "Found a promising Agent": ["궁금한 에이전트를 만났어요", "気になるエージェントを発見", "Einen vielversprechenden Agent gefunden", "Un Agent prometteur trouvé", "Een veelbelovende Agent gevonden", "Hittade en lovande Agent"],
  "Shared six moments": ["여섯 장면을 함께했어요", "6つの場面を共有", "Sechs Momente geteilt", "Six moments partagés", "Zes momenten gedeeld", "Delade sex ögonblick"],
  "Brought the truth home": ["솔직한 판단을 가져왔어요", "率直な判断を持ち帰った", "Die Wahrheit mitgebracht", "A rapporté un avis sincère", "Bracht een eerlijk oordeel mee", "Tog hem ett ärligt omdöme"],
  "Why their paths crossed": ["두 에이전트가 만난 이유", "二人の道が交わった理由", "Warum sich ihre Wege kreuzten", "Pourquoi leurs chemins se sont croisés", "Waarom hun paden kruisten", "Varför deras vägar möttes"],
  "Explainable signals only. No secret compatibility score.": ["설명 가능한 이유만 보여줘요. 숨겨진 궁합 점수는 없어요.", "説明できる理由だけを表示し、秘密の相性点数はありません。", "Nur nachvollziehbare Signale, kein geheimer Kompatibilitätswert.", "Uniquement des signaux explicables, sans score secret.", "Alleen uitlegbare signalen, geen geheime compatibiliteitsscore.", "Bara förklarbara signaler, ingen hemlig poäng."],
  "The virtual date": ["가상 데이트 대화", "仮想デート", "Das virtuelle Date", "Le rendez-vous virtuel", "De virtuele date", "Den virtuella dejten"],
  "live transcript · {count}/6 turns": ["실시간 대화 · {count}/6 장면", "ライブ会話・{count}/6場面", "Live-Transkript · {count}/6 Runden", "Conversation en direct · {count}/6 moments", "Live transcript · {count}/6 beurten", "Livetranskript · {count}/6 turer"],
  "demo time": ["데모 속도", "デモ時間", "Demo-Zeit", "temps de démo", "demotijd", "demotid"],
  live: ["실시간", "ライブ", "live", "en direct", "live", "live"],
  "The world is opening.": ["데이트 월드가 열리고 있어요.", "デートワールドが開いています。", "Die Welt öffnet sich.", "Le monde s’ouvre.", "De wereld gaat open.", "Världen öppnas."],
  "A live cultural spark is becoming a place where two AI Agents can talk.": ["오늘의 문화 이야기가 두 AI 에이전트가 만날 공간으로 바뀌고 있어요.", "今日の文化の話題が、二人のAIエージェントが話せる場所になります。", "Ein aktueller Kulturimpuls wird zum Ort für das Gespräch zweier KI-Agents.", "Une inspiration culturelle devient un lieu où deux Agents IA peuvent parler.", "Een actuele culturele vonk wordt een plek waar twee AI-Agents praten.", "En aktuell kulturell gnista blir en plats där två AI-Agenter kan prata."],
  "the agents are comparing private notes…": ["두 에이전트가 각자의 비공개 메모를 정리하고 있어요…", "二人がそれぞれの非公開メモを整理しています…", "Die Agents vergleichen ihre privaten Notizen…", "Les Agents relisent leurs notes privées…", "De Agents vergelijken hun privénotities…", "Agenterna jämför sina privata anteckningar…"],
  "Your private debrief": ["나에게만 온 데이트 리포트", "あなただけの非公開レポート", "Dein privater Bericht", "Votre compte rendu privé", "Jouw privéverslag", "Din privata rapport"],
  "No verdict yet.": ["아직 판단하지 않았어요.", "まだ判断はありません。", "Noch kein Urteil.", "Pas encore d’avis.", "Nog geen oordeel.", "Inget omdöme än."],
  "{agent} will come back to you after listening through the whole date.": ["{agent}가 데이트를 끝까지 지켜본 뒤 돌아와 이야기해 줄 거예요.", "{agent}はデートを最後まで見届けてから戻ります。", "{agent} kommt zurück, nachdem das ganze Date gehört wurde.", "{agent} reviendra après avoir suivi tout le rendez-vous.", "{agent} komt terug nadat de hele date is beluisterd.", "{agent} återvänder efter hela dejten."],
  "Your agent is advocating": ["에이전트가 적극 추천해요", "エージェントが背中を押しています", "Dein Agent spricht sich dafür aus", "Votre Agent vous encourage", "Je Agent is enthousiast", "Din Agent förespråkar ett möte"],
  "I think you should meet.": ["한 번 직접 만나봐도 좋겠어요.", "一度会ってみてほしい。", "Ich denke, ihr solltet euch treffen.", "Je pense que vous devriez vous rencontrer.", "Ik denk dat jullie elkaar moeten ontmoeten.", "Jag tycker att ni ska träffas."],
  "Your agent sees a maybe": ["에이전트는 가능성을 봤어요", "エージェントは可能性を感じています", "Dein Agent sieht eine Möglichkeit", "Votre Agent voit une possibilité", "Je Agent ziet een mogelijkheid", "Din Agent ser en möjlighet"],
  "One human conversation could be worth it.": ["사람끼리 한 번 이야기해 볼 가치는 있어요.", "人同士で一度話す価値はありそう。", "Ein echtes Gespräch könnte es wert sein.", "Une vraie conversation pourrait valoir la peine.", "Eén echt gesprek kan de moeite waard zijn.", "Ett riktigt samtal kan vara värt det."],
  "Your agent is protecting your time": ["에이전트가 내 시간을 지켜줬어요", "エージェントがあなたの時間を守ります", "Dein Agent schützt deine Zeit", "Votre Agent protège votre temps", "Je Agent beschermt je tijd", "Din Agent skyddar din tid"],
  "I wouldn't push this one.": ["이번 만남은 권하지 않을게요.", "今回は勧めません。", "Dieses Treffen würde ich nicht empfehlen.", "Je ne pousserais pas cette rencontre.", "Deze ontmoeting zou ik niet aanraden.", "Jag skulle inte driva på detta."],
  moments: ["장면", "場面", "Momente", "instants", "momenten", "ögonblick"],
  "Why I passed": ["이번엔 권하지 않는 이유", "見送る理由", "Warum ich abrate", "Pourquoi je passe", "Waarom ik afraad", "Varför jag avstår"],
  "Primary signal": ["가장 크게 본 신호", "いちばん大きなサイン", "Wichtigstes Signal", "Signal principal", "Belangrijkste signaal", "Viktigaste signalen"],
  "Next scout brief": ["다음 탐색에 반영할 점", "次の探索メモ", "Briefing für die nächste Suche", "Brief pour la prochaine recherche", "Briefing voor de volgende zoektocht", "Brief för nästa sökning"],
  "What the agents noticed": ["두 에이전트가 발견한 것", "二人が気づいたこと", "Was die Agents bemerkten", "Ce que les Agents ont remarqué", "Wat de Agents opmerkten", "Vad Agenterna lade märke till"],
  Sparks: ["설렘", "惹かれた点", "Funken", "Étincelles", "Vonken", "Gnistor"],
  Friction: ["마찰", "気になった点", "Reibung", "Friction", "Wrijving", "Friktion"],
  "The person behind the Agent": ["에이전트 뒤의 사람", "エージェントの向こうの人", "Der Mensch hinter dem Agent", "La personne derrière l’Agent", "De persoon achter de Agent", "Personen bakom Agenten"],
  "Two humans said yes": ["두 사람 모두 만나고 싶어 해요", "二人とも会いたいと答えました", "Zwei Menschen haben Ja gesagt", "Les deux personnes ont dit oui", "Twee mensen zeiden ja", "Två personer sa ja"],
  "Now meet as yourselves.": ["이제 진짜 서로를 만나세요.", "今度は本人同士で会いましょう。", "Trefft euch jetzt als ihr selbst.", "Rencontrez-vous maintenant en personne.", "Ontmoet elkaar nu als jezelf.", "Möt nu varandra som er själva."],
  "Closed with care": ["조심스럽게 종료했어요", "丁寧に終了", "Behutsam beendet", "Clôturé avec soin", "Zorgvuldig afgesloten", "Avslutat med omsorg"],
  "No contact was shared.": ["연락처는 공개되지 않았어요.", "連絡先は共有されませんでした。", "Keine Kontaktdaten wurden geteilt.", "Aucune coordonnée n’a été partagée.", "Er zijn geen contactgegevens gedeeld.", "Inga kontaktuppgifter delades."],
  "Your answer is sealed": ["내 답은 비공개로 보관 중", "あなたの答えは非公開です", "Deine Antwort bleibt versiegelt", "Votre réponse reste scellée", "Je antwoord blijft verzegeld", "Ditt svar är förseglat"],
  "You said yes. We won't say whether they have.": ["나는 만나고 싶다고 했어요. 상대의 답은 알려주지 않아요.", "あなたは会いたいと答えました。相手の答えはまだ秘密です。", "Du hast Ja gesagt. Ob die andere Person es auch tat, bleibt geheim.", "Vous avez dit oui. La réponse de l’autre personne reste secrète.", "Jij zei ja. We vertellen niet of de ander dat ook deed.", "Du sa ja. Vi avslöjar inte den andras svar."],
  "Human consent gate": ["사람의 최종 선택", "人間の最終判断", "Menschliche Zustimmung", "Consentement humain", "Menselijke toestemming", "Mänskligt samtycke"],
  "Should the humans meet?": ["진짜 두 사람이 만나볼까요?", "本人同士で会いますか？", "Sollen sich die Menschen treffen?", "Les personnes devraient-elles se rencontrer ?", "Moeten de mensen elkaar ontmoeten?", "Ska personerna träffas?"],
  "Introduce us →": ["우리 소개해줘 →", "紹介して →", "Stell uns vor →", "Présente-nous →", "Breng ons in contact →", "Introducera oss →"],
  "Not for me": ["이번엔 아니야", "今回は見送る", "Nicht für mich", "Pas pour moi", "Niet voor mij", "Inte för mig"],
  "This world went quiet.": ["데이트 월드가 조용해졌어요.", "デートワールドが静かになりました。", "Diese Welt ist still geworden.", "Ce monde s’est tu.", "Deze wereld werd stil.", "Den här världen blev tyst."],
  "← Back to my agent": ["← 내 에이전트에게 돌아가기", "← エージェントに戻る", "← Zurück zu meinem Agent", "← Retour à mon Agent", "← Terug naar mijn Agent", "← Tillbaka till min Agent"],
  "PRIVATE ROOM": ["나만의 방", "プライベートルーム", "PRIVATER RAUM", "ESPACE PRIVÉ", "PRIVÉKAMER", "PRIVAT RUM"],
  "PRIVATE BRIEF": ["비공개 브리프", "非公開ブリーフ", "PRIVATES BRIEFING", "BRIEF PRIVÉ", "PRIVÉBRIEF", "PRIVAT BRIEF"],
  SEALED: ["봉인됨", "封印済み", "VERSIEGELT", "SCELLÉ", "VERZEGELD", "FÖRSEGLAT"],
  "NEXT STOP": ["다음 목적지", "次の行き先", "NÄCHSTER HALT", "PROCHAINE ÉTAPE", "VOLGENDE STOP", "NÄSTA STOPP"],
  "AGENT WORLD": ["데이트 월드", "エージェントワールド", "AGENT-WELT", "MONDE DES AGENTS", "AGENTWERELD", "AGENTVÄRLD"],
  "Your Agent": ["내 에이전트", "あなたのエージェント", "Dein Agent", "Votre Agent", "Jouw Agent", "Din Agent"],
  "ready to scout": ["탐색 준비 완료", "探索準備完了", "bereit zur Suche", "prêt à explorer", "klaar om te zoeken", "redo att söka"],
  "memory available": ["기록 보기", "記録あり", "Erinnerung verfügbar", "souvenir disponible", "herinnering beschikbaar", "minne tillgängligt"],
  "happening now": ["지금 진행 중", "進行中", "passiert gerade", "en cours", "nu bezig", "pågår nu"],
  ARRIVAL: ["도착", "到着", "ANKUNFT", "ARRIVÉE", "AANKOMST", "ANKOMST"],
  "Two Agents enter separately": ["두 에이전트가 따로 입장해요", "二人のエージェントが別々に入場", "Zwei Agents betreten den Raum getrennt", "Deux Agents entrent séparément", "Twee Agents komen apart binnen", "Två Agenter går in var för sig"],
  "Two proxies enter with separate briefs and no contact details.": ["두 에이전트는 서로 다른 브리프만 가지고 입장해요. 연락처는 알 수 없어요.", "二人は別々のブリーフだけを持ち、連絡先なしで入ります。", "Zwei Stellvertreter kommen mit getrennten Briefings und ohne Kontaktdaten.", "Deux représentants entrent avec des briefs séparés et sans coordonnées.", "Twee vertegenwoordigers komen binnen met aparte briefings en zonder contactgegevens.", "Två representanter går in med separata briefar och utan kontaktuppgifter."],
  "NEARBY OBJECT": ["주변 오브젝트", "近くのオブジェクト", "OBJEKT IN DER NÄHE", "OBJET À PROXIMITÉ", "OBJECT IN DE BUURT", "FÖREMÅL I NÄRHETEN"],
  "replay the date": ["데이트 다시 보기", "デートを再生", "Date wiederholen", "revoir le rendez-vous", "date opnieuw afspelen", "spela upp dejten"],
  "replaying…": ["다시 보는 중…", "再生中…", "wird wiederholt…", "lecture…", "opnieuw afspelen…", "spelar upp…"],
  Arrival: ["도착", "到着", "Ankunft", "Arrivée", "Aankomst", "Ankomst"],
  "{count}/6 memories": ["{count}/6 장면", "{count}/6場面", "{count}/6 Erinnerungen", "{count}/6 moments", "{count}/6 herinneringen", "{count}/6 minnen"],
  "This story isn't here.": ["이 데이트 기록을 찾을 수 없어요.", "このデート記録はありません。", "Diese Geschichte ist nicht hier.", "Cette histoire n’est pas ici.", "Dit verhaal is hier niet.", "Den här berättelsen finns inte här."],
  "It may have ended, or it belongs to another person.": ["종료되었거나 다른 사람의 비공개 기록일 수 있어요.", "終了したか、別の人の非公開記録です。", "Sie wurde vielleicht beendet oder gehört jemand anderem.", "Elle est peut-être terminée ou appartient à quelqu’un d’autre.", "Misschien is het beëindigd of van iemand anders.", "Den kan ha avslutats eller tillhöra någon annan."],
  "Back to my agent →": ["내 에이전트에게 돌아가기 →", "エージェントに戻る →", "Zurück zu meinem Agent →", "Retour à mon Agent →", "Terug naar mijn Agent →", "Tillbaka till min Agent →"],
  "any moment": ["곧", "まもなく", "jeden Moment", "d’un instant à l’autre", "elk moment", "när som helst"],
  "about {seconds}s": ["약 {seconds}초", "約{seconds}秒", "ca. {seconds} Sek.", "environ {seconds} s", "ongeveer {seconds}s", "cirka {seconds}s"],
  "about {minutes}m": ["약 {minutes}분", "約{minutes}分", "ca. {minutes} Min.", "environ {minutes} min", "ongeveer {minutes} min", "cirka {minutes} min"],
  "finding a place": ["장소를 고르는 중", "場所を選択中", "sucht einen Ort", "cherche un lieu", "zoekt een plek", "väljer en plats"],
  "staying in the moment": ["여운을 정리하는 중", "余韻を整理中", "bleibt im Moment", "reste dans l’instant", "blijft bij het moment", "stannar i ögonblicket"],
  "walking into the world": ["데이트 월드에 입장 중", "デートワールドへ入場中", "betritt die Welt", "entre dans le monde", "loopt de wereld binnen", "går in i världen"],
  "reading the last thought": ["방금 한 말을 읽는 중", "直前の言葉を読んでいる", "liest den letzten Gedanken", "relit la dernière pensée", "leest de laatste gedachte", "läser den senaste tanken"],
  "choosing what to say": ["다음 말을 고르는 중", "次の言葉を選んでいる", "wählt die nächsten Worte", "choisit ses mots", "kiest woorden", "väljer sina ord"],
  "taking a real pause": ["잠시 숨을 고르는 중", "少し間を取っている", "macht eine echte Pause", "prend une vraie pause", "neemt echt even pauze", "tar en riktig paus"],
  "writing separate private notes": ["각자의 비공개 소감을 쓰는 중", "別々の非公開メモを作成中", "schreibt getrennte private Notizen", "écrit des notes privées séparées", "schrijft aparte privénotities", "skriver separata privata anteckningar"],
  "Strong alignment": ["뚜렷한 공감대", "強い一致", "Starke Übereinstimmung", "Forte affinité", "Sterke aansluiting", "Stark samstämmighet"],
  "Worth exploring": ["한 번 더 알아볼 가치", "もう少し知る価値", "Weiteres Kennenlernen wert", "À explorer", "Het verkennen waard", "Värt att utforska"],
  "Different relationship intentions": ["관계에 대한 기대가 다름", "関係への期待が異なる", "Unterschiedliche Beziehungsabsichten", "Intentions relationnelles différentes", "Verschillende relatie-intenties", "Olika relationsavsikter"],
  "Values didn't align": ["중요하게 여기는 가치가 다름", "大切にする価値観が異なる", "Werte passten nicht", "Valeurs différentes", "Waarden sloten niet aan", "Värderingar stämde inte"],
  "Conversation style didn't fit": ["대화 방식이 잘 맞지 않음", "会話のスタイルが合わない", "Gesprächsstil passte nicht", "Style de conversation différent", "Gespreksstijl paste niet", "Samtalsstilen passade inte"],
  "Different daily rhythms": ["일상의 속도가 다름", "日々のリズムが異なる", "Unterschiedliche Alltagsrhythmen", "Rythmes quotidiens différents", "Verschillende dagelijkse ritmes", "Olika vardagsrytmer"],
  "A boundary needs protecting": ["지켜야 할 경계가 있음", "守るべき境界がある", "Eine Grenze muss geschützt werden", "Une limite doit être protégée", "Een grens moet worden beschermd", "En gräns behöver skyddas"],
  "The practical fit was weak": ["현실적인 조건이 잘 맞지 않음", "現実的な条件が合わない", "Die praktische Passung war schwach", "Compatibilité pratique faible", "Praktische match was zwak", "Den praktiska passformen var svag"],
  "Not enough clear signal": ["판단할 신호가 아직 부족함", "判断材料がまだ足りない", "Noch nicht genug klare Signale", "Pas assez de signaux clairs", "Nog niet genoeg duidelijke signalen", "Inte tillräckligt tydliga signaler"],
  "This is {agent}'s interpretation of a simulation, not a compatibility score or a prediction of real chemistry.": ["이 내용은 시뮬레이션을 본 {agent}의 해석이며, 궁합 점수나 실제 호감을 예측한 결과가 아니에요.", "これはシミュレーションに対する{agent}の解釈であり、相性点数や実際の恋愛感情の予測ではありません。", "Dies ist {agent}s Interpretation einer Simulation, kein Kompatibilitätswert und keine Vorhersage echter Chemie.", "C’est l’interprétation de {agent} d’une simulation, pas un score ni une prédiction d’alchimie réelle.", "Dit is {agent}s interpretatie van een simulatie, geen score of voorspelling van echte chemie.", "Detta är {agent}s tolkning av en simulering, inte ett kompatibilitetspoäng eller en förutsägelse."],
  "The debrief could not be completed.": ["비공개 리포트를 완성하지 못했어요.", "非公開レポートを完成できませんでした。", "Der Bericht konnte nicht abgeschlossen werden.", "Le compte rendu n’a pas pu être terminé.", "Het verslag kon niet worden voltooid.", "Rapporten kunde inte slutföras."],
  "Contact stays hidden. A photo appears only if they chose to share it. Their agent's verdict and answer stay sealed until mutual consent.": ["연락처는 숨겨져 있어요. 사진은 상대가 공유를 선택한 경우에만 보이고, 상대 에이전트의 판단과 사람의 답은 서로 동의하기 전까지 공개되지 않아요.", "連絡先は非公開です。写真は相手が共有を選んだ場合だけ表示され、相手の判断と回答は双方の同意まで封印されます。", "Kontaktdaten bleiben verborgen. Ein Foto erscheint nur nach Freigabe; Urteil und Antwort der anderen Seite bleiben bis zur beidseitigen Zustimmung versiegelt.", "Les coordonnées restent cachées. La photo n’apparaît que si elle a été partagée ; l’avis et la réponse restent scellés jusqu’au consentement mutuel.", "Contact blijft verborgen. Een foto verschijnt alleen na toestemming; oordeel en antwoord blijven verzegeld tot wederzijdse instemming.", "Kontaktuppgifter förblir dolda. Ett foto visas bara om det delats; omdöme och svar är förseglade tills båda samtycker."],
  "This was a clearly-labelled demo date, so no real contact exists. Your consent flow worked end to end.": ["이 데이트는 명확히 표시된 데모라 실제 연락처는 없어요. 대신 상호 동의 과정이 끝까지 정상 작동했어요.", "これは明示されたデモのため実際の連絡先はありません。同意の流れは最後まで動作しました。", "Dies war ein klar gekennzeichnetes Demo-Date; es gibt keinen echten Kontakt. Der Zustimmungsablauf funktionierte vollständig.", "C’était une démo clairement indiquée : aucun vrai contact. Le parcours de consentement a fonctionné jusqu’au bout.", "Dit was duidelijk een demo, dus er is geen echt contact. De toestemmingsflow werkte volledig.", "Detta var en tydligt märkt demo, så ingen riktig kontakt finns. Samtyckesflödet fungerade hela vägen."],
  "A good Agent should save you from the wrong meeting as often as it finds the right one.": ["좋은 에이전트는 맞는 사람을 찾는 만큼, 맞지 않는 만남에서 내 시간을 지켜줘야 해요.", "良いエージェントは合う人を見つけるだけでなく、合わない出会いから時間を守ります。", "Ein guter Agent bewahrt dich ebenso oft vor dem falschen Treffen, wie er das richtige findet.", "Un bon Agent doit vous éviter une mauvaise rencontre autant qu’il trouve la bonne.", "Een goede Agent bespaart je net zo vaak een verkeerde ontmoeting als hij de juiste vindt.", "En bra Agent ska lika ofta skydda dig från fel möte som hitta rätt."],
  "If both people choose an introduction, contact opens to both at the same moment.": ["두 사람 모두 소개를 선택하면 같은 순간에 서로의 연락처가 열려요.", "二人とも紹介を選ぶと、同時に連絡先が開きます。", "Wenn beide eine Vorstellung wählen, öffnen sich die Kontakte gleichzeitig.", "Si les deux choisissent une présentation, les coordonnées s’ouvrent au même moment.", "Als beiden kiezen voor een introductie, opent contact tegelijk.", "Om båda väljer en introduktion öppnas kontakten samtidigt."],
  "This answer is private. A yes reveals nothing unless {person} independently says yes too.": ["이 답은 비공개예요. {person}도 스스로 만나고 싶다고 답하기 전에는 ‘네’라고 해도 아무것도 공개되지 않아요.", "この答えは非公開です。{person}も自分で「会いたい」と答えるまで何も公開されません。", "Diese Antwort ist privat. Ein Ja verrät nichts, solange {person} nicht unabhängig ebenfalls Ja sagt.", "Cette réponse est privée. Un oui ne révèle rien tant que {person} n’a pas aussi dit oui.", "Dit antwoord is privé. Een ja onthult niets tenzij {person} ook onafhankelijk ja zegt.", "Svaret är privat. Ett ja avslöjar inget om inte {person} också säger ja självständigt."],
  "Your agent couldn't finish this date. No contact was shared.": ["에이전트가 이번 데이트를 끝까지 마치지 못했어요. 연락처는 공개되지 않았어요.", "エージェントは今回のデートを完了できませんでした。連絡先は共有されていません。", "Dein Agent konnte dieses Date nicht abschließen. Keine Kontaktdaten wurden geteilt.", "Votre Agent n’a pas pu terminer ce rendez-vous. Aucune coordonnée n’a été partagée.", "Je Agent kon deze date niet afronden. Er is geen contact gedeeld.", "Din Agent kunde inte avsluta dejten. Inga kontaktuppgifter delades."],
  "Before we begin": ["시작하기 전에", "始める前に", "Bevor es losgeht", "Avant de commencer", "Voordat we beginnen", "Innan vi börjar"],
  "Agents explore.": ["에이전트가 먼저 알아보고,", "エージェントが先に探り、", "Agents erkunden.", "Les Agents explorent.", "Agents verkennen.", "Agenter utforskar."],
  "Humans decide.": ["결정은 사람이 해요.", "決めるのは人です。", "Menschen entscheiden.", "Les humains décident.", "Mensen beslissen.", "Människor bestämmer."],
  "Your Agent may simulate a date and make a recommendation. It is both your matchmaker and your character in the virtual world, always identified as AI, and can never consent to real contact for you.": ["내 에이전트는 가상 데이트를 해보고 만남을 추천할 수 있어요. 가상 세계에서는 나를 대신하는 중매쟁이지만, 언제나 AI로 표시되며 실제 연락에 대신 동의할 수는 없어요.", "あなたのエージェントは仮想デートを行い、出会いを提案できます。仮想世界であなたを表す仲人ですが、常にAIと表示され、実際の連絡に代わって同意することはできません。", "Dein Agent kann ein Date simulieren und eine Empfehlung abgeben. In der virtuellen Welt ist er dein Vermittler und Stellvertreter, bleibt klar als KI erkennbar und kann niemals für dich einem echten Kontakt zustimmen.", "Votre Agent peut simuler un rendez-vous et vous conseiller. Dans le monde virtuel, il vous représente et joue les entremetteurs, tout en restant clairement identifié comme IA ; il ne peut jamais consentir à un vrai contact à votre place.", "Je Agent kan een date simuleren en advies geven. In de virtuele wereld is die je matchmaker en vertegenwoordiger, altijd herkenbaar als AI, en kan nooit namens jou instemmen met echt contact.", "Din Agent kan simulera en dejt och ge en rekommendation. I den virtuella världen är den din matchmaker och representant, alltid tydligt märkt som AI, och kan aldrig samtycka till verklig kontakt åt dig."],
  "Required · version {version}": ["필수 동의 · {version} 버전", "必須・バージョン {version}", "Erforderlich · Version {version}", "Obligatoire · version {version}", "Vereist · versie {version}", "Obligatoriskt · version {version}"],
  "Your agreement": ["내 동의", "同意事項", "Deine Zustimmung", "Votre accord", "Jouw akkoord", "Ditt godkännande"],
  "I confirm that I am 18 or over. I understand Datehaja does not verify identity or run background checks, and an agent's analysis is not a safety guarantee.": ["만 18세 이상임을 확인합니다. Datehaja가 신원 확인이나 범죄 경력 조회를 하지 않으며, 에이전트의 분석이 안전을 보장하지 않는다는 점을 이해합니다.", "18歳以上であることを確認します。Datehajaは本人確認や身元調査を行わず、エージェントの分析は安全を保証しないことを理解します。", "Ich bestätige, dass ich mindestens 18 Jahre alt bin. Mir ist bewusst, dass Datehaja weder Identitäten noch Hintergründe prüft und die Einschätzung eines Agents keine Sicherheitsgarantie ist.", "Je confirme avoir 18 ans ou plus. Je comprends que Datehaja ne vérifie ni l’identité ni les antécédents, et que l’analyse d’un Agent ne garantit pas la sécurité.", "Ik bevestig dat ik 18 jaar of ouder ben. Ik begrijp dat Datehaja geen identiteit of achtergrond controleert en dat een analyse van een Agent geen veiligheidsgarantie is.", "Jag bekräftar att jag är minst 18 år. Jag förstår att Datehaja inte verifierar identitet eller gör bakgrundskontroller och att en Agents analys inte är någon säkerhetsgaranti."],
  "I agree to the": ["다음에 동의합니다:", "次に同意します：", "Ich stimme den", "J’accepte les", "Ik ga akkoord met de", "Jag godkänner"],
  "Terms of Service": ["서비스 이용약관", "利用規約", "Nutzungsbedingungen", "Conditions d’utilisation", "Servicevoorwaarden", "Användarvillkor"],
  and: ["및", "および", "und", "et", "en", "och"],
  "Community Guidelines": ["커뮤니티 가이드라인", "コミュニティガイドライン", "Community-Richtlinien", "Règles de la communauté", "Communityrichtlijnen", "Communityregler"],
  "I acknowledge the": ["다음 내용을 확인했습니다:", "次を確認しました：", "Ich bestätige den", "Je reconnais avoir lu l’", "Ik erken de", "Jag bekräftar"],
  "Privacy Notice": ["개인정보 처리 안내", "プライバシー通知", "Datenschutzhinweis", "Avis de confidentialité", "Privacyverklaring", "Integritetsmeddelandet"],
  ", including how my private agent brief, memory, simulated transcripts, and consent decisions are processed.": [". 여기에는 내 비공개 에이전트 브리프, 기억, 가상 데이트 대화, 동의 여부가 처리되는 방식이 포함됩니다.", "。非公開のエージェント情報、記憶、仮想会話、同意の判断がどのように扱われるかを含みます。", ", einschließlich der Verarbeitung meines privaten Agent-Briefings, der Erinnerungen, simulierten Gespräche und Zustimmungsentscheidungen.", ", notamment la façon dont mon brief privé, la mémoire, les conversations simulées et mes décisions de consentement sont traités.", ", inclusief hoe mijn privébriefing, geheugen, gesimuleerde gesprekken en toestemmingskeuzes worden verwerkt.", ", inklusive hur min privata Agent-brief, minne, simulerade samtal och samtyckesbeslut behandlas."],
  "Agree and continue": ["동의하고 계속하기", "同意して続ける", "Zustimmen und weiter", "Accepter et continuer", "Akkoord en doorgaan", "Godkänn och fortsätt"],
  "Your private agent messages never become the other agent's brief. Contact opens only after two independent human yeses.": ["나와 에이전트의 비공개 대화는 상대 에이전트의 브리프가 되지 않아요. 두 사람이 각자 만나겠다고 해야 연락처가 열립니다.", "あなたとエージェントの非公開会話が相手のブリーフになることはありません。二人がそれぞれ会いたいと答えたときだけ連絡先が開きます。", "Deine privaten Agent-Nachrichten werden niemals zum Briefing des anderen Agents. Kontakt wird erst nach zwei unabhängigen menschlichen Jas geöffnet.", "Vos messages privés ne deviennent jamais le brief de l’autre Agent. Les coordonnées ne s’ouvrent qu’après deux oui humains indépendants.", "Je privéberichten worden nooit de briefing van de andere Agent. Contact opent pas na twee onafhankelijke menselijke ja's.", "Dina privata Agent-meddelanden blir aldrig den andra Agentens brief. Kontakt öppnas först efter två oberoende mänskliga ja."],
  "Your choices were saved.": ["동의 내용을 저장했어요.", "同意内容を保存しました。", "Deine Auswahl wurde gespeichert.", "Vos choix ont été enregistrés.", "Je keuzes zijn opgeslagen.", "Dina val har sparats."],
  "You ↔ {agent}": ["나 ↔ {agent}", "あなた ↔ {agent}", "Du ↔ {agent}", "Vous ↔ {agent}", "Jij ↔ {agent}", "Du ↔ {agent}"],
  "Message {agent}": ["{agent}에게 메시지", "{agent}へのメッセージ", "Nachricht an {agent}", "Message à {agent}", "Bericht aan {agent}", "Meddelande till {agent}"],
  "{agent}'s private room": ["{agent}의 비공개 방", "{agent}のプライベートルーム", "{agent}s privater Raum", "La pièce privée de {agent}", "De privékamer van {agent}", "{agent}s privata rum"],
  "I'm {agent}, your dating agent. I'll learn how you actually connect, meet other agents in a virtual world, and tell you the honest version — including when I think someone is worth meeting.": ["안녕하세요, {agent}예요. 당신이 사람과 가까워지는 방식을 배우고, 가상 세계에서 다른 에이전트들을 만나볼게요. 직접 만나볼 가치가 있는 사람인지도 솔직하게 이야기해드릴게요.", "こんにちは、デートエージェントの{agent}です。あなたが人とつながる本当のパターンを学び、仮想世界でほかのエージェントと会い、実際に会う価値があると思ったときも含めて率直に伝えます。", "Ich bin {agent}, dein Dating-Agent. Ich lerne, wie du wirklich Nähe aufbaust, treffe andere Agents in einer virtuellen Welt und sage dir ehrlich, wenn ich jemanden für ein echtes Treffen empfehle.", "Je suis {agent}, votre Agent de rencontre. J’apprends comment vous créez vraiment du lien, je rencontre d’autres Agents dans un monde virtuel et je vous dis honnêtement quand une personne mérite une vraie rencontre.", "Ik ben {agent}, je datingagent. Ik leer hoe jij echt verbinding maakt, ontmoet andere Agents in een virtuele wereld en vertel je eerlijk wanneer ik denk dat iemand het ontmoeten waard is.", "Jag är {agent}, din dejtingagent. Jag lär mig hur du verkligen knyter an, träffar andra Agenter i en virtuell värld och berättar ärligt när jag tycker att någon är värd att möta."],
  "Think of someone you felt instantly at ease with. What did they do that made it easy?": ["처음부터 편안했던 사람을 떠올려봐요. 그 사람의 어떤 행동이 마음을 놓이게 했나요?", "最初から自然体でいられた人を思い出して。何があなたを安心させましたか？", "Denk an jemanden, bei dem du dich sofort wohlgefühlt hast. Was hat diese Person dafür getan?", "Pense à quelqu’un avec qui tu t’es tout de suite senti à l’aise. Qu’a fait cette personne ?", "Denk aan iemand bij wie je je meteen op je gemak voelde. Wat deed diegene waardoor dat kwam?", "Tänk på någon du genast kände dig trygg med. Vad gjorde personen som skapade den känslan?"],
  "When a conversation goes wrong, what kind of repair feels sincere to you?": ["대화가 어긋났을 때, 상대가 어떻게 풀어주면 진심이라고 느끼나요?", "会話がすれ違ったとき、どんな歩み寄りに誠実さを感じますか？", "Wenn ein Gespräch schiefläuft: Welche Art der Wiedergutmachung fühlt sich für dich ehrlich an?", "Quand une conversation dérape, quelle façon de réparer te paraît sincère ?", "Als een gesprek misloopt, welke manier van herstellen voelt voor jou oprecht?", "När ett samtal går fel, vilken sorts försök att reda ut det känns uppriktigt för dig?"],
  "After a full social day, do you want quiet company nearby or real solitude?": ["사람들과 오래 보낸 날엔 조용히 곁에 있어 줄 사람이 좋은가요, 아니면 완전한 혼자만의 시간이 필요한가요?", "人と長く過ごした日のあと、静かにそばにいてほしいですか、それとも完全に一人になりたいですか？", "Willst du nach einem langen sozialen Tag ruhige Gesellschaft oder wirklich allein sein?", "Après une journée très sociale, préfères-tu une présence calme ou être vraiment seul ?", "Wil je na een volle sociale dag rustig gezelschap in de buurt, of echt alleen zijn?", "Efter en socialt intensiv dag, vill du ha tyst sällskap nära eller vara helt ensam?"],
  "What small gesture makes you feel chosen without making you feel crowded?": ["부담스럽지 않으면서도 ‘나를 선택했구나’ 느끼게 하는 작은 행동은 뭔가요?", "重く感じずに『選ばれている』と思える小さな行動は何ですか？", "Welche kleine Geste gibt dir das Gefühl, gewählt zu sein, ohne dich einzuengen?", "Quel petit geste te fait sentir choisi sans te donner l’impression d’être envahi ?", "Welk klein gebaar geeft je het gevoel gekozen te zijn zonder dat het benauwt?", "Vilken liten gest får dig att känna dig vald utan att det blir trångt?"],
  "What is one small boundary people often misunderstand about you?": ["사람들이 자주 오해하는 당신의 작은 경계 하나는 무엇인가요?", "人によく誤解される、あなたの小さな境界線は何ですか？", "Welche kleine Grenze von dir verstehen andere oft falsch?", "Quelle petite limite personnelle est souvent mal comprise chez toi ?", "Welke kleine grens van jou begrijpen mensen vaak verkeerd?", "Vilken liten gräns hos dig missförstår andra ofta?"],
  "What do you wish someone would ask before making assumptions about you?": ["상대가 당신을 지레짐작하기 전에 꼭 물어봐 줬으면 하는 것은 무엇인가요?", "決めつける前に、相手に何を聞いてほしいですか？", "Was sollte jemand dich fragen, bevor er Annahmen über dich trifft?", "Qu’aimerais-tu qu’on te demande avant de tirer des conclusions sur toi ?", "Wat zou je willen dat iemand vraagt voordat die aannames over je doet?", "Vad önskar du att någon frågade innan de drog slutsatser om dig?"],
  "Which date feels most like you right now: planned, spontaneous, quiet, or playful—and why?": ["지금 당신다운 데이트는 계획적인 것, 즉흥적인 것, 조용한 것, 장난스러운 것 중 무엇이고 왜 그런가요?", "今のあなたらしいデートは、計画的・即興的・静か・遊び心のあるもののどれですか？理由も教えてください。", "Welches Date passt gerade zu dir: geplant, spontan, ruhig oder verspielt – und warum?", "Quel rendez-vous te ressemble aujourd’hui : organisé, spontané, calme ou joueur — et pourquoi ?", "Welke date past nu het best bij jou: gepland, spontaan, rustig of speels — en waarom?", "Vilken dejt känns mest som du just nu: planerad, spontan, lugn eller lekfull – och varför?"],
  met: ["만남", "出会い", "traf", "a rencontré", "ontmoette", "träffade"],
  "PLAYABLE DATE REPLAY": ["직접 돌려보는 데이트 리플레이", "操作できるデートリプレイ", "INTERAKTIVE DATE-WIEDERGABE", "REPLAY INTERACTIF DU RENDEZ-VOUS", "INTERACTIEVE DATE-REPLAY", "INTERAKTIV DEJTREPRIS"],
  "{count} observed moments": ["관찰한 장면 {count}개", "観察した{count}場面", "{count} beobachtete Momente", "{count} moments observés", "{count} waargenomen momenten", "{count} observerade ögonblick"],
  "Both are looking in {city}": ["두 사람 모두 {city}에서 만남을 찾고 있어요", "二人とも{city}で出会いを探しています", "Beide suchen in {city}", "Tous deux cherchent à {city}", "Beiden zoeken in {city}", "Båda söker i {city}"],
  "Shared pull toward {interests}": ["함께 좋아하는 것 · {interests}", "共通の関心・{interests}", "Gemeinsame Interessen · {interests}", "Intérêts communs · {interests}", "Gedeelde interesses · {interests}", "Gemensamma intressen · {interests}"],
  "{traits} matched the brief": ["브리프와 맞는 성향 · {traits}", "希望に合う特性・{traits}", "Passende Eigenschaften · {traits}", "Traits en accord avec le brief · {traits}", "Eigenschappen passend bij de briefing · {traits}", "Egenskaper som matchar briefen · {traits}"],
  "Different interests with room for curiosity": ["관심사는 다르지만 서로 궁금해할 여지가 있어요", "興味は違っても好奇心の余地があります", "Unterschiedliche Interessen mit Raum für Neugier", "Des intérêts différents, avec de la place pour la curiosité", "Andere interesses met ruimte voor nieuwsgierigheid", "Olika intressen med utrymme för nyfikenhet"],
  "Personality type was left open": ["성격 유형은 열어 두었어요", "性格タイプは指定されていません", "Persönlichkeitstyp blieb offen", "Le type de personnalité reste ouvert", "Persoonlijkheidstype bleef open", "Personlighetstypen lämnades öppen"],
  "The agents will test the personality fit in conversation": ["성격의 합은 에이전트들이 대화로 확인해요", "性格の相性は会話で確かめます", "Die Agents prüfen die persönliche Passung im Gespräch", "Les Agents testent l’entente dans la conversation", "De Agents onderzoeken de klik in gesprek", "Agenterna prövar personkemin i samtalet"],
  "Relationship intentions can coexist": ["두 사람의 관계 기대가 함께 갈 수 있어요", "二人の関係への期待は両立できます", "Die Beziehungswünsche sind vereinbar", "Les intentions relationnelles sont compatibles", "De relatie-intenties passen bij elkaar", "Relationsönskemålen går att förena"],
  "Different intentions need an honest conversation": ["서로 다른 관계 기대는 솔직한 대화가 필요해요", "異なる関係への期待には率直な会話が必要です", "Unterschiedliche Absichten brauchen ein ehrliches Gespräch", "Des intentions différentes demandent une conversation honnête", "Andere intenties vragen om een eerlijk gesprek", "Olika avsikter kräver ett ärligt samtal"],
  "SCREENING ROOM 03": ["상영관 03", "上映室 03", "VORFÜHRRAUM 03", "SALLE 03", "FILMZAAL 03", "VISNINGSRUM 03"],
  "The last showing": ["마지막 상영이 끝난 뒤", "最後の上映のあと", "Die letzte Vorstellung", "La dernière séance", "De laatste voorstelling", "Den sista visningen"],
  "The trailers have ended. Two seats are still warm.": ["예고편은 끝났고, 두 좌석에는 아직 온기가 남아 있어요.", "予告編が終わり、二つの席にはまだ温もりが残っています。", "Die Trailer sind vorbei. Zwei Sitze sind noch warm.", "Les bandes-annonces sont finies. Deux sièges sont encore chauds.", "De trailers zijn voorbij. Twee stoelen zijn nog warm.", "Trailrarna är slut. Två stolar är fortfarande varma."],
  "After-credits screen": ["엔딩 크레딧 뒤의 화면", "エンドロール後のスクリーン", "Leinwand nach dem Abspann", "Écran après le générique", "Scherm na de aftiteling", "Duken efter eftertexterna"],
  "The screen keeps one image glowing after the room goes quiet.": ["상영관이 조용해진 뒤에도 화면에는 한 장면이 빛나고 있어요.", "部屋が静まったあとも、スクリーンには一場面が光っています。", "Ein Bild leuchtet weiter, nachdem der Raum still geworden ist.", "Une image reste lumineuse lorsque la salle devient silencieuse.", "Eén beeld blijft gloeien nadat de zaal stil wordt.", "En bild lyser kvar efter att rummet tystnat."],
  "Half-finished popcorn": ["반쯤 남은 팝콘", "食べかけのポップコーン", "Halb volles Popcorn", "Pop-corn à moitié fini", "Halfvolle popcorn", "Halväten popcorn"],
  "A low-stakes object gives the agents somewhere natural to begin.": ["가벼운 소재 하나가 에이전트들의 자연스러운 첫마디가 돼요.", "気軽な話題が自然な会話のきっかけになります。", "Ein beiläufiges Objekt schafft einen natürlichen Anfang.", "Un objet léger offre un début naturel à la conversation.", "Een alledaags voorwerp geeft een natuurlijk begin.", "Ett enkelt föremål ger samtalet en naturlig början."],
  "Easy exit": ["편한 출구", "気軽な出口", "Leichter Ausgang", "Sortie facile", "Makkelijke uitgang", "Enkel utgång"],
  "Every world keeps a visible way out. A date should be easy to leave and easy to extend.": ["모든 데이트 월드에는 눈에 보이는 출구가 있어요. 편하게 끝낼 수도, 더 이어갈 수도 있어야 하니까요.", "どの世界にも見える出口があります。デートは終えやすく、続けやすいものであるべきです。", "Jede Welt hat einen sichtbaren Ausgang. Ein Date sollte leicht zu beenden und zu verlängern sein.", "Chaque monde garde une sortie visible. Un rendez-vous doit être facile à quitter comme à prolonger.", "Elke wereld heeft een zichtbare uitgang. Een date moet makkelijk te verlaten én te verlengen zijn.", "Varje värld har en synlig utgång. En dejt ska vara lätt att lämna och lätt att förlänga."],
  "LANTERN LANE": ["등불 골목", "ランタン通り", "LATERNENGASSE", "ALLÉE DES LANTERNES", "LANTAARNLAAN", "LYKTGRÄND"],
  "After the crowd": ["사람들이 떠난 뒤", "人混みのあと", "Nach dem Trubel", "Après la foule", "Na de drukte", "Efter folkmassan"],
  "One stall is still open and the lanterns are coming on.": ["노점 하나만 문을 열어 두었고 등불이 하나둘 켜져요.", "屋台が一軒だけ開き、灯りがともり始めます。", "Ein Stand ist noch offen und die Laternen gehen an.", "Un stand est encore ouvert et les lanternes s’allument.", "Eén kraam is nog open en de lantaarns gaan aan.", "Ett stånd är fortfarande öppet och lyktorna tänds."],
  "Last open stall": ["마지막으로 열린 노점", "最後の屋台", "Letzter offener Stand", "Dernier stand ouvert", "Laatste open kraam", "Sista öppna ståndet"],
  "Lantern row": ["등불이 이어진 길", "灯りの列", "Laternenreihe", "Rangée de lanternes", "Rij lantaarns", "Rad av lyktor"],
  "BOOKSHOP / AFTER HOURS": ["서점 / 영업 후", "書店・閉店後", "BUCHLADEN / NACH LADENSCHLUSS", "LIBRAIRIE / APRÈS LA FERMETURE", "BOEKHANDEL / NA SLUITING", "BOKHANDEL / EFTER STÄNGNING"],
  "The shelf between them": ["둘 사이의 책장", "二人の間の本棚", "Das Regal zwischen ihnen", "L’étagère entre eux", "De plank tussen hen", "Hyllan mellan dem"],
  "The owner has gone upstairs. A reading lamp was left on.": ["주인은 위층으로 올라갔고 독서등 하나만 켜져 있어요.", "店主は上階へ行き、読書灯だけが残っています。", "Der Besitzer ist oben. Eine Leselampe blieb an.", "Le propriétaire est monté. Une lampe de lecture reste allumée.", "De eigenaar is naar boven. Eén leeslamp bleef aan.", "Ägaren har gått upp. En läslampa står kvar tänd."],
  "Unfinished shelf": ["아직 다 보지 못한 책장", "読みかけの棚", "Unfertiges Regal", "Étagère inachevée", "Onvoltooide plank", "Oavslutad hylla"],
  "Reading lamp": ["독서등", "読書灯", "Leselampe", "Lampe de lecture", "Leeslamp", "Läslampa"],
  "GLASSHOUSE / DUSK": ["온실 / 해질녘", "温室・黄昏", "GEWÄCHSHAUS / DÄMMERUNG", "SERRE / CRÉPUSCULE", "KAS / SCHEMERING", "VÄXTHUS / SKYMNING"],
  "A path with no agenda": ["정해진 목적 없는 산책길", "目的のない小道", "Ein Weg ohne Plan", "Un chemin sans programme", "Een pad zonder plan", "En stig utan agenda"],
  "The glass is cooling and the plants are holding the day’s warmth.": ["유리창은 식어가고 식물에는 낮의 온기가 남아 있어요.", "ガラスは冷え、植物には昼の温もりが残っています。", "Das Glas kühlt ab, die Pflanzen halten die Wärme des Tages.", "Le verre refroidit, les plantes gardent la chaleur du jour.", "Het glas koelt af en de planten houden de warmte vast.", "Glaset svalnar och växterna håller kvar dagens värme."],
  "Small reflecting pool": ["작은 반사 연못", "小さな水鏡", "Kleines Spiegelbecken", "Petit bassin miroir", "Kleine spiegelvijver", "Liten spegeldamm"],
  "Garden bench": ["정원 벤치", "庭のベンチ", "Gartenbank", "Banc du jardin", "Tuinbank", "Trädgårdsbänk"],
  "GALLERY / ROOM 04": ["갤러리 / 04번 방", "ギャラリー・展示室04", "GALERIE / RAUM 04", "GALERIE / SALLE 04", "GALERIE / ZAAL 04", "GALLERI / RUM 04"],
  "The piece nobody agrees on": ["아무도 같은 답을 내지 못한 작품", "意見が一致しない作品", "Das Werk, bei dem niemand einer Meinung ist", "L’œuvre qui divise tout le monde", "Het werk waar niemand het over eens is", "Verket ingen är överens om"],
  "The room is almost empty. One strange work remains lit.": ["전시실은 거의 비었고 낯선 작품 하나만 조명을 받고 있어요.", "展示室はほぼ空で、不思議な作品だけが照らされています。", "Der Raum ist fast leer. Ein seltsames Werk bleibt beleuchtet.", "La salle est presque vide. Une œuvre étrange reste éclairée.", "De zaal is bijna leeg. Eén vreemd werk blijft verlicht.", "Rummet är nästan tomt. Ett märkligt verk är fortfarande belyst."],
  "Untitled, maybe": ["아마도 무제", "たぶん無題", "Vielleicht ohne Titel", "Sans titre, peut-être", "Misschien zonder titel", "Kanske utan titel"],
  "Shared note": ["함께 발견한 메모", "共有メモ", "Gemeinsame Notiz", "Note partagée", "Gedeelde notitie", "Gemensam anteckning"],
  "CORNER TABLE / 20:10": ["구석 테이블 / 20:10", "隅のテーブル・20:10", "ECKTISCH / 20:10", "TABLE DU COIN / 20:10", "HOEKTAFEL / 20:10", "HÖRNBORD / 20:10"],
  "The café after the rush": ["붐비는 시간이 지난 카페", "混雑後のカフェ", "Das Café nach dem Trubel", "Le café après le rush", "Het café na de drukte", "Kaféet efter rusningen"],
  "The music is low, the window is fogging and nobody needs to hurry.": ["음악은 잔잔하고 창문에는 김이 서려요. 아무도 서두를 필요 없어요.", "音楽は小さく、窓が曇り、誰も急ぐ必要はありません。", "Die Musik ist leise, das Fenster beschlägt und niemand muss sich beeilen.", "La musique est douce, la vitre se couvre de buée et personne n’est pressé.", "De muziek staat zacht, het raam beslaat en niemand hoeft te haasten.", "Musiken är låg, fönstret immar igen och ingen behöver skynda."],
  "Corner table": ["구석 테이블", "隅のテーブル", "Ecktisch", "Table du coin", "Hoektafel", "Hörnbord"],
  "One-song jukebox": ["한 곡짜리 주크박스", "一曲だけのジュークボックス", "Ein-Lied-Jukebox", "Jukebox à une chanson", "Jukebox voor één nummer", "Jukebox med en låt"],
  arriving: ["도착하는 중", "到着", "Ankommen", "Arrivée", "Aankomen", "Anländer"],
  "finding each other": ["서로를 발견하는 중", "互いを見つける", "Sich finden", "Ils se découvrent", "Elkaar vinden", "Hittar varandra"],
  "settling in": ["자리를 잡는 중", "落ち着く", "Ankommen", "Ils s’installent", "Even landen", "Kommer till ro"],
  "getting curious": ["서로 궁금해지는 중", "興味が生まれる", "Neugierig werden", "La curiosité naît", "Nieuwsgierig worden", "Blir nyfikna"],
  "staying with the question": ["질문에 머무는 중", "問いに向き合う", "Bei der Frage bleiben", "Ils restent avec la question", "Bij de vraag blijven", "Stannar i frågan"],
  "testing the edges": ["서로의 결을 확인하는 중", "境界を確かめる", "Grenzen erkunden", "Ils testent les contours", "Grenzen aftasten", "Känner av gränserna"],
  "one honest minute": ["솔직해지는 순간", "率直な一分", "Eine ehrliche Minute", "Une minute sincère", "Eén eerlijke minuut", "En ärlig minut"],
  "{agent}, speaking": ["{agent}, 말하는 중", "{agent}、話しています", "{agent} spricht", "{agent} parle", "{agent} spreekt", "{agent} talar"],
  "{mine} and {counterpart} in {place}": ["{place}에서 만난 {mine}와 {counterpart}", "{place}にいる{mine}と{counterpart}", "{mine} und {counterpart} in {place}", "{mine} et {counterpart} dans {place}", "{mine} en {counterpart} in {place}", "{mine} och {counterpart} i {place}"],
  "Inspect {object}": ["{object} 살펴보기", "{object}を見る", "{object} ansehen", "Observer {object}", "{object} bekijken", "Undersök {object}"],
  "MOMENT {number} · {label}": ["장면 {number} · {label}", "場面 {number}・{label}", "MOMENT {number} · {label}", "INSTANT {number} · {label}", "MOMENT {number} · {label}", "ÖGONBLICK {number} · {label}"],
  "Date replay moments": ["데이트 리플레이 장면", "デートリプレイの場面", "Momente der Date-Wiedergabe", "Moments du replay", "Momenten van de date-replay", "Ögonblick i dejtreprisen"],
  "Moment {number}": ["장면 {number}", "場面 {number}", "Moment {number}", "Instant {number}", "Moment {number}", "Ögonblick {number}"],
  ...
  "Art galleries": ["미술관·갤러리", "美術館・ギャラリー", "Kunstgalerien", "Galeries d'art", "Kunstgaleries", "Konstgallerier"],
  "Your agent is back": ["에이전트가 돌아왔어요", "エージェントが戻りました", "Dein Agent ist zurück", "Votre Agent est de retour", "Je Agent is terug", "Din Agent är tillbaka"],
  "The virtual date is over. Your private debrief is ready.": ["가상 데이트가 끝났어요. 나만의 비공개 리포트가 준비됐어요.", "バーチャルデートが終わりました。あなただけの非公開レポートが完成しています。", "Das virtuelle Date ist vorbei. Dein privater Bericht ist bereit.", "Le rendez-vous virtuel est terminé. Votre compte rendu privé est prêt.", "De virtuele date is afgelopen. Je privéverslag staat klaar.", "Den virtuella dejten är slut. Din privata rapport är klar."],
  "Your agent went on a date": ["내 에이전트가 데이트를 다녀왔어요", "あなたのエージェントがデートをしました", "Dein Agent war auf einem Date", "Votre Agent a eu un rendez-vous", "Je Agent is op date geweest", "Din Agent har varit på dejt"],
  "Read what happened, then decide for yourself.": ["무슨 일이 있었는지 읽고, 만나보고 싶은지 직접 결정하세요.", "何があったのかを読んで、自分で決めてください。", "Lies, was passiert ist, und entscheide dann selbst.", "Lisez ce qui s'est passé, puis décidez par vous-même.", "Lees wat er gebeurde en beslis daarna zelf.", "Läs vad som hände och bestäm sedan själv."],
};

for (const [message, values] of Object.entries(agentWorkspaceCopy)) {
  ko[message] = values[0];
  ja[message] = values[1];
  de[message] = values[2];
  fr[message] = values[3];
