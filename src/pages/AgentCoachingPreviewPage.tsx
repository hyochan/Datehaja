...
export default function AgentCoachingPreviewPage() {
  ...
  return <main className="date-record-preview learning-proof">
    ...
    <section className="date-preview-intro">
      ...
      <nav className="date-record-nav"><a href="#comparison">{say("바뀐 말투 비교하기 ↓", "Compare before and after ↓")}</a><a href="#coaching">{say("실제로 남긴 피드백 ↓", "Read the saved feedback ↓")}</a><a href="#dates">{say("네 번의 데이트 살펴보기 ↓", "Explore all four dates ↓")}</a><a href="https://youtu.be/qu1VWBXuIz8" target="_blank" rel="noreferrer">{say("전체 시연 보기 ↗", "Watch the full walkthrough ↗")}</a></nav><p className="proof-caption">{say("한국어 원문입니다. English를 누르면 같은 기록의 번역을 볼 수 있어요.", "English translation of Korean records. Switch to 한국어 원문 to read the unchanged source. This is recorded evidence, not a new live generation.")}</p>
    </section>
