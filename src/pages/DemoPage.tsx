...
export default function DemoPage() {
  ...
  return <main className="submission-page">
    ...
    <video className="submission-video" controls playsInline preload="metadata" poster={demoAsset("submission-poster.jpg")} aria-label="Datehaja product demonstration with English captions">
      <source src={demoAsset("Datehaja-demo.mp4")} type="video/mp4" />
      <track kind="captions" src={demoAsset("Datehaja-demo.vtt")} srcLang="en" label="English" />
      {say("아래 링크에서 영상을 내려받을 수 있어요.", "Download the video using the link below.")}
    </video>
    <nav className="submission-links"><a href={demoAsset("Datehaja-demo.mp4")} download>{say("영상 내려받기", "Download video")}</a><a href={demoAsset("Datehaja-demo.vtt")} download>{say("영어 자막", "English captions")}</a><a href={demoAsset("transcript.txt")}>{say("시연 대본 읽기", "Read the walkthrough transcript")}</a></nav>
